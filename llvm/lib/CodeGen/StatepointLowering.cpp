    ...
    for (MachineInstr &MI : *MBB) {
      if (MI.getOpcode() == TargetOpcode::RELOCATE_DEF) {
        ...
        while (DefOfSrc) {
          ...
          if (!MaybeSrc)
            break;
          if (!TargetRegisterInfo::isVirtualRegister(MaybeSrc))
            break;
          Src = MaybeSrc;
    ...
    for (auto It = PHIs.begin(); It != PHIs.end();) {
      ...
      for (unsigned I = 1, E = MPhi->getNumOperands(); I < E; I += 2) {
        ...
        if (!TargetRegisterInfo::isVirtualRegister(Src))
          continue;
        unsigned Dst = MaybeCopy->getOperand(0).getReg();
      ...
      unsigned FirstReg;
      unsigned MPhiDef = MPhi->getOperand(0).getReg();
      bool MPhiOneDef = MRI->hasOneDef(MPhiDef);
      for (unsigned I = 1, E = MPhi->getNumOperands(); I < E; I += 2) {
        unsigned MaybeFirstReg = MPhi->getOperand(I).getReg();
