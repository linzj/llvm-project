...
// Out-of-line definitions of Array's non-template members. They are only
// declared inside class Array because std::vector<Value> member functions
// require Value to be complete (enforced by newer libc++ versions).
...
inline Array::iterator Array::insert(iterator P, const Value &E) {
  return V.insert(P, E);
}
inline Array::iterator Array::insert(iterator P, Value &&E) {
